...
def register_images(img, label_img, atlas_img):
    ...
    registered_img = sitk.Resample(image1=img,referenceImage=atlas_img, transform=transform, interpolator=sitk.sitkLinear,\
    defaultPixelValue=0.0, outputPixelType=img.GetPixelIDValue())

    registered_label = sitk.Resample(image1=label_img,referenceImage=atlas_img, transform=transform,interpolator=sitk.sitkNearestNeighbor, defaultPixelValue=0.0,\
...
def preprocess_rescale_numpy(np_img, new_min_val, new_max_val):
    max_val = np_img.max()
    min_val = np_img.min()
    normalized = (np_img - min_val)/(max_val-min_val)  # todo: modify here


    rescaled_np_img = normalized*(new_max_val-new_min_val) + new_min_val
